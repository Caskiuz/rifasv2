...
export default function Registro() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (res.ok) setMsg('Registro exitoso, ahora puedes iniciar sesión.');
  ...
  return (
    <form className="bg-white p-6 rounded shadow max-w-md mx-auto" onSubmit={handleSubmit} noValidate>
      ...
      <input className={`w-full mb-2 p-2 border rounded ${errors.name ? 'border-red-500' : ''}`} name="name" placeholder="Nombre" value={form.name} onChange={handleChange} required />
      ...
      <button className="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-700 transition" type="submit">Registrarse</button>
